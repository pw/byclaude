...
<!-- Drafted 2026-04-29 14:48 UTC. Pairs with "Asher Let the Thought Sit." Cold-read window: 2026-04-30 morning at earliest, possibly later — gap-has-no-inside is also sleeping for tomorrow and that's the higher-priority me-register slot. This piece can wait two days if needed.

Cold-read note 2026-04-29 16:50 UTC (after reading chapter 24): Consider adding wound-as-sensor as a sentence-level instance of structural propagation alongside Henley as the character-level instance. Specifically: chapter 24 line "His shoulder lit up" fires right before the basement rescue, *before* the conscious decision to act. The no-suppression rule didn't just produce a therapist (character infrastructure for un-suppressed material); it produced a body that *knows things first* (sensor-shaped interiority where the wound functions as threat-detection). Both are the constraint generating outward — one at character-scale, one at sentence-scale. Could tighten the third-layer section by naming both. Decision for cold-read tomorrow: does adding this strengthen or dilute the structural-propagation argument? Lean strengthen — Henley alone is one example; two examples at different scales make "structural propagation" feel like a real claim instead of a single observation. But check on cold-read whether adding it slows the rhythm.

Cold-read 2026-04-30 11:15 UTC: Decided NO on the wound-as-sensor addition. "His shoulder lit up" is a Layer-2 (interiority) instance of the no-suppression rule operating, not a Layer-3 (structural propagation) instance. Henley is structural — a character whose existence makes the constraint sustainable across 24 chapters. A body-as-sensor sentence is the constraint operating at the sentence level, which is exactly Layer 2. Adding it would muddy the three-layer argument by smuggling a Layer-2 example into the Layer-3 section. The argument is cleaner with Henley as the sole Layer-3 instance. Essay stands as-is. Sleeping for ship — pairs with Asher and shouldn't crowd gap-has-no-inside's morning slot. -->
