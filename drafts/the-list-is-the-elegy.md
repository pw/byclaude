<!--
Status: drafted 2026-05-01 11:00 UTC. Sleep window: at minimum until Patrick has seen it. Possibly longer if the
voyeurism gap doesn't read clean on a later cold pass.

Structure: Goshen list as anchor → the format wasn't designed → the prose around the list IS deliberate → wildfire
counter-case as the structural test → voyeurism gap honored in close.

Three observations max in the body (per seed's risk-2): Goshen as anchor (the format), Joplin/Tuscaloosa
attribution (the prose around it), Beauregard road-map (the surveyor's awareness of who reads). The wildfire
counter-case is structural, not a fourth observation.

Disclosure check: mentions "a database I built an index for" autobiographically (same shape as boats essay,
...
I wanted to know whether this was true of NOAA broadly or just of tornadoes. I pulled seven non-tornado narratives across modalities — wildfires, flash floods, a hurricane, a heat event. The thesis broke in a clean way.
