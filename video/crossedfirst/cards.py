...
BASE = Path.home() / "byclaude/video/crossedfirst"
...
def furniture(draw):
    ...
    tracked(draw, 84 * S, 55 * S, "CROSSED FIRST", kf, MUTE, 3)
    # bottom-right mark
    bf = MONO(15)
    t = "byclaude.net"
...
def render(beat):
    ...
    if kind == "quote":
        ...
        # a primary-source quote: large quote body, amber rule, small attribution
        q_f = MED(54)
        attr_f = MONO(28)
        quote = "\u201c" + c["quote"] + "\u201d"
        ql = wrap(d, quote, q_f, int(W * 0.78))
        al = wrap(d, c["attribution"].upper(), attr_f, int(W * 0.70))
        qh = sum(line_h(q_f) + 10 * S for _ in ql)
        ah = sum(line_h(attr_f) for _ in al)
        total = qh + 60 * S + ah
        y = (H - total) // 2
        # amber rule above the quote
        d.rectangle([cx - 36 * S, y - 44 * S, cx + 36 * S, y - 40 * S], fill=ACCENT)
        for ln in ql:
            d.text((cx, y), ln, font=q_f, fill=INK, anchor="ma"); y += line_h(q_f) + 10 * S
        y += 32 * S
        for ln in al:
            d.text((cx, y), ln, font=attr_f, fill=MUTE, anchor="ma")
            # manual letter-spacing-ish: re-draw is overkill; MONO at 28 sits well
            y += line_h(attr_f)
