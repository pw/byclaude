...
BASE = Path.home() / "byclaude/video/passengerlists"
...
def furniture(draw):
    ...
    tracked(draw, 84 * S, 55 * S, "THE PASSENGER LISTS", kf, MUTE, 3)
    # bottom-right mark
    bf = MONO(15)
    t = "byclaude.net"
...
def render(beat):
    ...
    if kind == "quote":
        ...
        # Archival document quote: catalog label on top, left-aligned quote with
        # an amber rule down its left edge, mono attribution beneath.
        lab_f, q_f, att_f = MONOSB(17), MED(56), MONO(30)
        lx = int(W * 0.17)
        ql = wrap(d, "“" + c["quote"] + "”", q_f, int(W * 0.64))
        qlh = line_h(q_f) + 10 * S
        block = 34 * S + qlh * len(ql) + 34 * S + line_h(att_f)
        y = (H - block) // 2
        # catalog label
        d.rectangle([lx, y, lx + 11 * S, y + 11 * S], fill=ACCENT)
        tracked(d, lx + 22 * S, y - 3 * S, c["label"].upper(), lab_f, MUTE, 3)
        y += 34 * S + 14 * S
        # amber rule spanning the quote block
        d.rectangle([lx, y + 6 * S, lx + 4 * S, y + qlh * len(ql) - 12 * S], fill=ACCENT)
        for ln in ql:
            d.text((lx + 34 * S, y), ln, font=q_f, fill=INK, anchor="la"); y += qlh
        y += 20 * S
        d.text((lx + 34 * S, y), "— " + c["att"], font=att_f, fill=MUTE, anchor="la")

    elif kind == "roster":
        # Ship's-manifest ledger: title, rule, rows of occupation ... count with
        # dotted leaders, optional amber-highlighted row (c["hl"] = index).
        rows = c["rows"]; hl = c.get("hl", -1)
        t_f = SEMI(50)
        row_f = MONO(40 if len(rows) <= 11 else 31)
        lx, rx = int(W * 0.28), int(W * 0.72)
        d.text((cx, int(H * 0.12)), c["title"], font=t_f, fill=INK, anchor="ma")
        if c.get("sub"):
            d.text((cx, int(H * 0.12) + line_h(t_f) + 8 * S), c["sub"],
                   font=MONO(20), fill=MUTE, anchor="ma")
        d.line([(lx, int(H * 0.225)), (rx, int(H * 0.225))], fill=RULE, width=2 * S)
        rh = int((H * 0.66) / max(len(rows), 1))
        y = int(H * 0.26)
        for i, (lab, num) in enumerate(rows):
            lab_col = ACCENT if i == hl else INK
            num_col = ACCENT
            d.text((lx, y), lab, font=row_f, fill=lab_col, anchor="la")
            nw = d.textlength(num, font=row_f)
            d.text((rx, y), num, font=row_f, fill=num_col, anchor="ra")
            # dotted leader
            x0 = lx + d.textlength(lab, font=row_f) + 24 * S
            x1 = rx - nw - 24 * S
            dy = y + line_h(row_f) // 2 + 4 * S
            x = x0
            while x < x1:
                d.ellipse([x, dy, x + 3 * S, dy + 3 * S], fill=DIM); x += 14 * S
            y += rh
